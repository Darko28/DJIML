...
class MPSYOLO {
    ...
    public init(commandQueue: MTLCommandQueue) {
        ...
        /*
         ...
         pixel format is .unorm8 and the channel format for the graph's input image
         is .float16, Metal will automatically convert the pixels to be between 0 and 1.
         */
        let inputImage = MPSNNImageNode(handle: nil)
        
        let scale = MPSNNLanczosScaleNode(source: inputImage, outputSize: MTLSize(width: MPSYOLO.inputWidth, height: MPSYOLO.inputHeight, depth: 3))
        
        let conv1 = MPSCNNConvolutionNode(source: scale.resultImage, weights: DataSource("conv1", 3, 3, 3, 16))
        
        let pool1 = MPSCNNPoolingMaxNode(source: conv1.resultImage, filterSize: 2)
        
        let conv2 = MPSCNNConvolutionNode(source: pool1.resultImage, weights: DataSource("conv2", 3, 3, 16, 32))
        
        let pool2 = MPSCNNPoolingMaxNode(source: conv2.resultImage, filterSize: 2)
        
        let conv3 = MPSCNNConvolutionNode(source: pool2.resultImage, weights: DataSource("conv3", 3, 3, 32, 64))
        
        let pool3 = MPSCNNPoolingMaxNode(source: conv3.resultImage, filterSize: 2)
        
        let conv4 = MPSCNNConvolutionNode(source: pool3.resultImage, weights: DataSource("conv4", 3, 3, 64, 128))
        
        let pool4 = MPSCNNPoolingMaxNode(source: conv4.resultImage, filterSize: 2)
        
        let conv5 = MPSCNNConvolutionNode(source: pool4.resultImage, weights: DataSource("conv5", 3, 3, 128, 256))
        
        let pool5 = MPSCNNPoolingMaxNode(source: conv5.resultImage, filterSize: 2)
        
        let conv6 = MPSCNNConvolutionNode(source: pool5.resultImage, weights: DataSource("conv6", 3, 3, 256, 512))
        ...
        let conv7 = MPSCNNConvolutionNode(source: pool6.resultImage, weights: DataSource("conv7", 3, 3, 512, 1024))
        
        let conv8 = MPSCNNConvolutionNode(source: conv7.resultImage, weights: DataSource("conv8", 3, 3, 1024, 1024))
        
        // The final convolution layer uses a 1x1 kernel and no activation function.
        let conv9 = MPSCNNConvolutionNode(source: conv8.resultImage, weights: DataSource("conv9", 1, 1, 1024, 125, useLeaky: false))
        
        if let graph = MPSNNGraph(device: commandQueue.device, resultImage: conv9.resultImage) {
            self.graph = graph
        } else {
...
//        graph.options = .verbose
    ...
    class DataSource: NSObject, MPSCNNConvolutionDataSource {
        ...
        init(_ name: String, _ kernelWidth: Int, _ kernelHeight: Int, _ inputFeatureChannels: Int, _ outputFeatureChannels: Int, useLeaky: Bool = true) {
            self.name = name
        ...
        func descriptor() -> MPSCNNConvolutionDescriptor {
            ...
            if useLeaky {
                ...
                data?.withUnsafeBytes { (ptr: UnsafePointer<Float>) -> Void in
                    let weightSize = outputFeatureChannels * kernelHeight * kernelWidth * inputFeatureChannels
                    let mean = ptr.advanced(by: weightSize)
                    ...
                    desc.setBatchNormalizationParametersForInferenceWithMean(mean, variance: variance, gamma: gamma, beta: beta, epsilon: 1e-3)
                }
            } else {
    ...
    func computeBoundingBoxes(_ image: MPSImage) -> [Prediction] {
        ...
        // We only run the convolutional part pf YOLO on the GPU. The last past of
        // the process is done on the CPU. It should be possible to do this on the GPU too,
        ...
        // This helper function finds the offset in the features array for a given channel
        // for a particular pixel. (See the comment below.)
        ...
        /*
         ...
         predict 5 bounding boxes (boxesPerCell). A bounding box consists of five data
         items: x, y, width, height, and a confidence score. Each grid cell also predicts
         which class each bounding box belongs to.
         
         The "features" array therefore contains (numClasses + 5)*boxesPerCell values for
         each grid cell, i.e. 125 channels. The total features array contains 13x13x125
         elements (actually x128 instead of x125 because in Metal the number of channels must be a multiple of 4).
         */
        ...
        for cy in 0..<gridHeight {
            for cx in 0..<gridWidth {
                for b in 0..<boxesPerCell {
                    ...
                    /*
                     The 13x13x125 image is arranged in planes of 4 channels.
                     First are channels 0-3 for the entire image, then channels 4-7 for the whole image, then channels 8-11, and so on.
                     Since we have 128 channels, there are 128/4 = 32 of these planes (a.k.a. texture slices).
                     ...
                     these 25 channels are spread out over multiple slices. We use a helper
                     function to find the correct place in the features array.
                     (Note: It might be quicker / more convenient to transpose this array
                     so that all 125 channels are stored consecutively instead of being scattered over multiple texture slices.)
                     */
                    ...
                    /*
                     The predicted tx and ty cooridnates are relative to the location
                     of the gird cell; we use the logistic sigmoid to constrain these
                    ...
                    /*
                     ...
                     how likely it is that there is an object in this box (but not what kind of object it is), with the largest class prediction, which tells us what kind of object it detected (but not where).
                     */
         ...
         */
        return nonMaxSuppression1(boxes: predictions, limit: MPSYOLO.maxBoundingBoxes, threshold: iouThreshold)
    }
}

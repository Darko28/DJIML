...
extension MTLTexture {
    ...
    public func toFloat16Array(width: Int, height: Int, featureChannels: Int) -> [Float16] {
        return toArray(width: width, height: height, featureChannels: featureChannels, initial: Float16(0))
    }
    ...
    /**
     ...
     The type of `initial` determines the type of the output array. In the following
     example, the type of bytes is `[UInt8]`.
     ...
     - Parameters:
        ...
        - initial: This parameter is necessary because we need to give the array an initial
     value. Unfortunately, we can't do `[T](repeating: T(0), ...)` since `T` cound be anything and may not have an init that takes a literal value.
     */
    func toArray<T>(width: Int, height: Int, featureChannels: Int, initial: T) -> [T] {
        ...
        getBytes(&bytes, bytesPerRow: width * featureChannels * MemoryLayout<T>.stride, from: region, mipmapLevel: 0)
        return bytes
    }
}

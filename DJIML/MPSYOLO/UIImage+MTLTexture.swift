...
extension UIImage {
    ...
    /**
     Converts an MTLTexture into an UIImage. This is useful for debugging.
     ...
     - Note: For `.float16` textures the pixels are expected to be in the range
        0...1; if you're using a different range (e.g. 0...255) then you have to specify
        a `scale` factor and possibly an `offset`. Alternatively, you can use an `MPSNeuronLinear`
        to scale the pixels down first.
     */
    
    @nonobjc public class func image(texture: MTLTexture, scale: Float = 1, offset: Float = 0) -> UIImage {
    ...
    @nonobjc class func image(textureRGBA16Float texture: MTLTexture, scale: Float = 1, offset: Float = 0) -> UIImage {
        ...
        // Finally, turn the byte array into an UIImage.
        return UIImage.fromByteArray(&outputRGBA, width: w, height: h)
    }
    
    @nonobjc class func image(textureR16Float texture: MTLTexture, scale: Float = 1, offset: Float = 0) -> UIImage {
